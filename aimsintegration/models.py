...
class QatarCrewBasic(models.Model):
    ...
    flight_no = models.CharField(max_length=6)
    tail_no = models.CharField(max_length=10, null=True, blank=True)
    flight_date = models.DateField()  # Scheduled departure date
    origin = models.CharField(max_length=4)  # ICAO code
    destination = models.CharField(max_length=4)  # ICAO code
    crew_id = models.CharField(max_length=20)
    position = models.CharField(max_length=10)  # PS field
    ...
    def __str__(self):
        return f"{self.crew_id} - Flight {self.flight_no} on {self.flight_date}"


class QatarCrewDetailed(models.Model):
    """Model for JOB 1008 - Detailed crew passport information"""
    crew_id = models.CharField(max_length=20)
    passport_number = models.CharField(max_length=20)
    ...
    def __str__(self):
        return f"{self.crew_id} - {self.given_name} {self.surname}"


class QatarApisRecord(models.Model):
    """Model to store generated APIS records"""
    DIRECTION_CHOICES = [
        ('I', 'Inbound'),
        ('O', 'Outbound'),
    ]
    ...
    def __str__(self):
        return f"APIS - {self.flight_no} ({self.direction}) - {self.given_name} {self.family_name}"