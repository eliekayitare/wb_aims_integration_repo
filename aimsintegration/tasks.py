...
# ===========================
#  QATAR APIS – SAVE-ONLY VERSION (no sending)
# ===========================
import os
import io
import csv
...
def _parse_date(s: str, fmts: Iterable[str] = ("%d/%m/%Y", "%Y-%m-%d", "%y%m%d", "%d%m%Y")) -> Optional[date]:
    s = (s or "").strip()
    if not s:
        return None
    for f in fmts:
...
def _parse_time(s: str, fmts: Iterable[str] = ("%H:%M", "%H%M")) -> Optional[time]:
    s = (s or "").strip()
    if not s:
        return None
    for f in fmts:
...
def _combine(d: Optional[date], t: Optional[time]) -> Optional[datetime]:
    if not d or not t:
        return None
    return timezone.make_aware(datetime.combine(d, t), timezone=timezone.utc)


def _determine_direction(dep_port: str, arr_port: str) -> str:
    """Inbound(I) when arriving DOH, else Outbound(O)."""
    return 'I' if (arr_port or '').upper() == 'DOH' else 'O'


def _gen_refs() -> Tuple[str, str, str]:
    ts = timezone.now().strftime("%y%m%d%H%M%S")
    return ts, ts, ts


def _qatar_apis_output_dir() -> str:
    base = getattr(settings, "QATAR_APIS_OUTPUT_DIR",
                   os.path.join(settings.MEDIA_ROOT, "qatar_apis"))
    os.makedirs(base, exist_ok=True)
    return base


def _qatar_apis_filename(dep_date: date, dep_port: str, arr_port: str) -> str:
    ts = timezone.now().strftime("%Y%m%d%H%M%S")
    # qatar_apis_scheduled_date_origin_destination_timestamp
    return f"qatar_apis_{dep_date.strftime('%Y%m%d')}_{dep_port}_{arr_port}_{ts}.edi"
...
def _build_edifact_paxlst(
    ...
    """
    Build a PAXLST D.05B message (TYPE=C crew).
    flight keys: flight_no, dep_iata, arr_iata, std_utc(dt aware), sta_utc(dt aware)
    person keys: surname, given_name, sex, birth_date(date),
                 nationality, document_type, passport_number,
                 issuing_state, passport_expiry_date(date), birth_country
    """
    std = flight["std_utc"].strftime("%y%m%d%H%M")
    sta = flight["sta_utc"].strftime("%y%m%d%H%M")
    unb_time = timezone.now().strftime("%y%m%d:%H%M")

    segs = []
    segs.append("UNA:+.?*'")
    segs.append(f"UNB+UNOA:4+RWANDAIR:ZZ+QATAPIS:ZZ+{unb_time}+{interchange_ref}'")
    segs.append(f"UNG+PAXLST+RWANDAIR:ZZ+QATAPIS:ZZ+{unb_time}+{group_ref}+UN+D:05B'")
    segs.append(f"UNH+{message_ref}+PAXLST:D:05B:UN:IATA+{flight['flight_no']}:{direction}+01:C'")
    segs.append("BGM+250'")
    segs.append(f"TDT+20+{flight['flight_no']}'")
    segs.append(f"LOC+125+{flight['dep_iata']}'")
    segs.append(f"DTM+189:{std}:201'")
    segs.append(f"LOC+87+{flight['arr_iata']}'")
    segs.append(f"DTM+232:{sta}:201'")

    for p in people:
        segs.append(f"NAD+FM+++{p['surname']}:{p['given_name']}+++++'")
        segs.append(f"ATT+2++{p['sex']}'")
        segs.append(f"DTM+329:{p['birth_date'].strftime('%y%m%d')}'")
        segs.append(f"LOC+22+{flight['arr_iata']}'")
        segs.append(f"LOC+178+{flight['dep_iata']}'")
        segs.append(f"LOC+179+{flight['arr_iata']}'")
        segs.append(f"NAT+2+{p['nationality']}'")
        segs.append(f"DOC+{p['document_type']}:110:111+{p['passport_number']}'")
        segs.append(f"DTM+36:{p['passport_expiry_date'].strftime('%y%m%d')}'")
        segs.append(f"LOC+91+{p['issuing_state']}'")

    segs.append(f"CNT+41:{len(people):04d}'")
    segs.append(f"UNT+{len(segs)+1}+{message_ref}'")
    segs.append(f"UNE+1+{group_ref}'")
    segs.append(f"UNZ+1+{interchange_ref}'")
    return "\n".join(segs)
...
def _lookup_flight_times(
    ...
    """
    Use FlightData to find STD/STA (UTC) for (flight_no, dep/arr IATA, dep_date).
    """
    try:
        qs = FlightData.objects.filter(
            flight_no=flight_no,
            dep_code_iata=dep_port_iata,
            arr_code_iata=arr_port_iata,
            sd_date_utc=dep_date
        ).order_by('std_utc')
        f = qs.first()
        if not f:
            return None, None
        std_dt = _combine(f.sd_date_utc, f.std_utc)
        sta_dt = _combine(f.sa_date_utc or f.sd_date_utc, f.sta_utc)
        return std_dt, sta_dt
    except Exception:
        logger.exception("Flight STD/STA lookup failed")
        return None, None
...
def _parse_job1008_attachment(att: FileAttachment) -> int:
    """
    Parse JOB1008 (Crew master) CSV and upsert CrewAPISMaster.
    """
    content = att.content.decode(errors="ignore")
    ...
    with transaction.atomic():
        for row in reader:
            try:
                crew_id         = (row.get('Crew - Crew Member ID') or row.get('Crew Member ID') or '').strip()
                if not crew_id:
                    continue

                surname         = (row.get('Crew - Passport Surname') or row.get('Surname') or '').strip()
                given_name      = (row.get('Crew - Passport FirstName') or row.get('FirstName') or '').strip()
                nationality     = (row.get('Crew - Nationality Of Person') or row.get('Nationality') or '').strip()[:3]
                passport_number = (row.get('Crew - Passport Number') or row.get('Passport Number') or '').strip()
                issuing_state   = (row.get('Crew - Passport Place Of Issue') or row.get('Issuing State') or '').strip()[:3]
                birth_date      = _parse_date(row.get('Crew - Birth Date') or row.get('Birth Date'))
                sex             = (row.get('Crew - Sex') or row.get('Sex') or '').strip()[:1]
                birth_country   = (row.get('Crew - Birth Place Country Code') or row.get('Birth Country Code') or '').strip()[:3]
                expiry_date     = _parse_date(row.get('Crew - Passport Expiry Date') or row.get('Passport Expiry Date'))
                doc_type        = (row.get('Crew - Document Type') or row.get('Document Type') or 'P').strip()[:1]
                issue_date      = _parse_date(row.get('Crew - Passport Issue Date') or row.get('Passport Issue Date'))

                if not (surname and given_name and passport_number and birth_date and expiry_date):
                    continue

                CrewAPISMaster.objects.update_or_create(
                    crew_id=crew_id,
                    defaults=dict(
                        surname=surname,
                        given_name=given_name,
                        nationality=nationality or 'RWA',
                        document_type=doc_type or 'P',
                        passport_number=passport_number,
                        issuing_state=issuing_state or nationality or 'RWA',
                        birth_date=birth_date,
                        passport_expiry_date=expiry_date,
                        sex=sex or 'M',
                        birth_country=birth_country or nationality or 'RWA',
                ...
                logger.exception("Failed to upsert JOB1008 row", extra={'row': row})
                continue
    return count


def _parse_job97_attachment(att: FileAttachment) -> List[Dict]:
    """
    Parse JOB97 (GD) file and return rows to store in Job97Record.
    """
    content = att.content.decode(errors="ignore")
    ...
    for row in reader:
        try:
            r = {
                "type": (row.get('TYPE') or 'C').strip()[:1],
                "direction": (row.get('DIRECTION') or '').strip()[:1],
                "flight_no": (row.get('FLIGHT') or '').strip(),
                "dep_port": (row.get('DEP PORT') or '').strip()[:3],
                "dep_date": _parse_date(row.get('DEP DATE')),
                "dep_time": _parse_time(row.get('DEP TIME')),
                "arr_port": (row.get('ARR PORT') or '').strip()[:3],
                "arr_date": _parse_date(row.get('ARR DATE')),
                "arr_time": _parse_time(row.get('ARR TIME')),
                "crew_id": (row.get('Crew Member ID') or row.get('CREW ID') or '').strip(),
                "birth_date": _parse_date(row.get('Date of Birth')),
                "sex": (row.get('Sex') or '').strip()[:1],
                "nationality": (row.get('Nationality') or '').strip()[:3],
                "raw_line": str(row)
            }
            if r["flight_no"] and r["dep_port"] and r["arr_port"] and r["dep_date"] and r["crew_id"]:
                rows.append(r)
        except Exception:
            logger.exception("Error parsing JOB97 row", extra={'row': row})
            continue
    return rows
...
def _process_email_attachment(item, process_fn):
    try:
        if item.attachments:
            for att in item.attachments:
                if isinstance(att, FileAttachment):
                    logger.info(f"Processing attachment: {att.name}")
                    return process_fn(att)
    except Exception:
        logger.exception("Error processing email attachment")
    return None
...
def fetch_job97_and_generate_qatar_apis():
    ...
    total_rows = 0
    for email in emails:
        rows = _process_email_attachment(email, _parse_job97_attachment)
        if not rows:
            continue

        # Persist rows to Job97Record
        with transaction.atomic():
            for r in rows:
                Job97Record.objects.update_or_create(
                    job97_id=email.message_id,  # exchangelib unique id
                    ...
                    defaults=dict(
                        type=r["type"],
                        direction=r["direction"] or _determine_direction(r["dep_port"], r["arr_port"]),
                        dep_time=r["dep_time"],
                        arr_date=r["arr_date"],
                        arr_time=r["arr_time"],
                        sex=r["sex"],
...
def _generate_qatar_apis_messages():
    """
    Group Job97Record by (flight_no, dep_port, arr_port, dep_date) and generate APIS if not already done.
    """
    to_process = (
        Job97Record.objects
        .values('flight_no', 'dep_port', 'arr_port', 'dep_date', 'direction')
        .order_by()
        .distinct()
    )

    output_dir = _qatar_apis_output_dir()

    for grp in to_process:
        flight_no  = grp['flight_no']
        dep_port   = grp['dep_port']
        arr_port   = grp['arr_port']
        dep_date   = grp['dep_date']
        direction  = grp['direction'] or _determine_direction(dep_port, arr_port)

        # Skip if already generated (simple existence check)
        if QatarAPISMessage.objects.filter(
            flight_no=flight_no,
            dep_port=dep_port,
            arr_port=arr_port,
            dep_date=dep_date
        ...
        # Fetch rows and master data
        records = Job97Record.objects.filter(
            flight_no=flight_no,
            dep_port=dep_port,
            arr_port=arr_port,
            dep_date=dep_date
        ...
        # Lookup STD/STA from FlightData
        std_dt, sta_dt = _lookup_flight_times(
            flight_no=flight_no,
            dep_port_iata=dep_port,
            arr_port_iata=arr_port,
            dep_date=dep_date
        )
        # Fallback to JOB97 schedule if not found
        if not std_dt:
            first = records.order_by('dep_time').first()
            std_dt = _combine(dep_date, first.dep_time) if first and first.dep_time else timezone.now()
        if not sta_dt:
            last = records.order_by('-arr_time').first()
            sta_dt = _combine(last.arr_date or dep_date, last.arr_time) if last and last.arr_time else std_dt
        ...
        # Build crew list using CrewAPISMaster
        people = []
        missing = []
        for r in records:
            master = CrewAPISMaster.objects.filter(crew_id=r.crew_id).first()
            if not master:
                missing.append(r.crew_id)
                continue
            people.append(dict(
                crew_id=r.crew_id,
                surname=master.surname,
                given_name=master.given_name,
                sex=(r.sex or master.sex or 'M')[:1],
                ...
                issuing_state=master.issuing_state,
                passport_expiry_date=master.passport_expiry_date,
                birth_country=master.birth_country,
            ))

        if not people:
            logger.warning(f"No crew found in master for {flight_no} {dep_port}-{arr_port} {dep_date}. Skipping.")
            continue

        interchange_ref, group_ref, message_ref = _gen_refs()
        edifact = _build_edifact_paxlst(flight_ctx, people, direction, interchange_ref, group_ref, message_ref)

        # ---------- Save ONLY (no sending) ----------
        filename = _qatar_apis_filename(dep_date, dep_port, arr_port)
        filepath = os.path.join(output_dir, filename)
        with open(filepath, "w", encoding="utf-8") as f:
            f.write(edifact)
        # -------------------------------------------

        # Persist that we created the file
        QatarAPISMessage.objects.create(
            job97_group=f"{flight_no}-{dep_port}-{arr_port}-{dep_date}",
            flight_no=flight_no,
            dep_port=dep_port,
            arr_port=arr_port,
            dep_date=dep_date,
            ...
            file_path=filepath,
            edifact_payload=edifact,
            missing_crew_ids=missing or None
        )

        logger.info(
            f"Generated & saved (NOT SENT) APIS for {flight_no} "
            f"{dep_port}-{arr_port} {dep_date} ({len(people)} crew) -> {filepath}"
        )
