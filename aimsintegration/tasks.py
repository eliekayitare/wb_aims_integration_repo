...
@shared_task(bind=True)
def cleanup_duplicate_flights(self, dry_run=False):
    """
    Celery task to clean up duplicate flight records.
    ...
    try:
        logger.info("=" * 60)
        logger.info("STARTING DUPLICATE FLIGHT CLEANUP TASK")
        logger.info("=" * 60)
        
        if dry_run:
            logger.warning("DRY RUN MODE - No changes will be made")
        
        # Create a report file for the cleanup operation
        timestamp = now().strftime("%Y%m%d_%H%M%S")
        report_filename = f"cleanup_duplicates_report_{timestamp}.txt"
        ...
        with open(report_path, 'w') as report_file:
            report_file.write(f"Duplicate Flight Cleanup Report\n")
            report_file.write(f"Generated: {now()}\n")
            report_file.write(f"Mode: {'DRY RUN' if dry_run else 'LIVE'}\n")
            report_file.write("=" * 60 + "\n\n")
            
            # Find all flights that have duplicates
            duplicates = FlightData.objects.values(
                'flight_no', 'sd_date_utc', 'dep_code_icao', 'arr_code_icao'
            ).annotate(
                count=Count('id')
            ).filter(count__gt=1).order_by('flight_no', 'sd_date_utc')
            
            total_duplicate_groups = duplicates.count()
            total_records_to_remove = sum(dup['count'] - 1 for dup in duplicates)
            
            logger.info(f"Found {total_duplicate_groups} flights with duplicates")
            logger.info(f"Total duplicate records to remove: {total_records_to_remove}")
            
            report_file.write(f"Summary:\n")
            report_file.write(f"- Flights with duplicates: {total_duplicate_groups}\n")
            report_file.write(f"- Total records to remove: {total_records_to_remove}\n\n")
            ...
            for duplicate_group in duplicates:
                flight_no = duplicate_group['flight_no']
                sd_date_utc = duplicate_group['sd_date_utc']
                dep_code_icao = duplicate_group['dep_code_icao']
                arr_code_icao = duplicate_group['arr_code_icao']
                count = duplicate_group['count']
                
                logger.info(f"Processing: Flight {flight_no} on {sd_date_utc} ({count} records)")
                
                report_file.write(f"\nFlight {flight_no} on {sd_date_utc} ({dep_code_icao} → {arr_code_icao})\n")
                report_file.write(f"Found {count} duplicate records:\n")
                ...
                for i, record in enumerate(records):
                    report_file.write(f"  Record {i+1} (ID: {record.id}):\n")
                    report_file.write(f"    Tail: {record.tail_no}, STD: {record.std_utc}, STA: {record.sta_utc}\n")
                    report_file.write(f"    ATD: {record.atd_utc}, Takeoff: {record.takeoff_utc}\n")
                    report_file.write(f"    Touchdown: {record.touchdown_utc}, ATA: {record.ata_utc}\n")
                
                if not dry_run:
                    try:
                        with transaction.atomic():
                            # Keep the first record as the master
                            master_record = records.first()
                            duplicate_records = records[1:]
                            
                            report_file.write(f"  → Keeping master record ID {master_record.id}\n")
                            logger.info(f"  Keeping master record ID {master_record.id}")
                            
                            # Merge data from duplicates into master record
                            merged_data = []
                            for duplicate in duplicate_records:
                                # Preserve any actual times that might be in duplicates
                                if not master_record.atd_utc and duplicate.atd_utc:
                                    master_record.atd_utc = duplicate.atd_utc
                                    merged_data.append(f"ATD: {duplicate.atd_utc}")
                                
                                if not master_record.takeoff_utc and duplicate.takeoff_utc:
                                    master_record.takeoff_utc = duplicate.takeoff_utc
                                    merged_data.append(f"Takeoff: {duplicate.takeoff_utc}")
                                
                                if not master_record.touchdown_utc and duplicate.touchdown_utc:
                                    master_record.touchdown_utc = duplicate.touchdown_utc
                                    merged_data.append(f"Touchdown: {duplicate.touchdown_utc}")
                                
                                if not master_record.ata_utc and duplicate.ata_utc:
                                    master_record.ata_utc = duplicate.ata_utc
                                    merged_data.append(f"ATA: {duplicate.ata_utc}")
                                
                                # Update tail number to latest if master doesn't have one
                                if not master_record.tail_no and duplicate.tail_no:
                                    master_record.tail_no = duplicate.tail_no
                                    merged_data.append(f"Tail: {duplicate.tail_no}")
                            
                            if merged_data:
                                report_file.write(f"  → Merged data: {', '.join(merged_data)}\n")
                                logger.info(f"  Merged data: {', '.join(merged_data)}")
                            
                            # Save master record with merged data
                            master_record.save()
                            
                            # Delete duplicates
                            duplicate_ids = [record.id for record in duplicate_records]
                            deleted_count = FlightData.objects.filter(id__in=duplicate_ids).delete()[0]
                            
                            report_file.write(f"  → Removed {deleted_count} duplicate records: {duplicate_ids}\n")
                        ...
                        error_msg = f"  ❌ Error processing flight {flight_no}: {e}"
                        logger.error(error_msg)
                        report_file.write(f"{error_msg}\n")
                        continue
                else:
                    report_file.write(f"  → DRY RUN: Would remove {count - 1} records\n")
                    records_removed += (count - 1)
                    flights_processed += 1
            
            # Write summary
            summary = f"\n{'='*60}\nFINAL SUMMARY:\n"
            summary += f"- Flights processed: {flights_processed}\n"
            summary += f"- Records removed: {records_removed}\n"
            summary += f"- Mode: {'DRY RUN' if dry_run else 'LIVE EXECUTION'}\n"
            summary += f"{'='*60}\n"
            
            report_file.write(summary)
            logger.info(summary)
        
        logger.info(f"✅ Duplicate cleanup completed. Report saved to: {report_path}")
        
        return {
            'status': 'success',
            'duplicates_found': total_duplicate_groups,
        ...
        error_msg = f"❌ Error in cleanup_duplicate_flights task: {e}"
...
@shared_task(bind=True)
def schedule_cleanup_duplicates(self, dry_run=True):
    """
    Wrapper task to easily schedule duplicate cleanup.
    Defaults to dry_run=True for safety.
    """
    logger.info("Scheduling duplicate cleanup task...")
    
    # Call the actual cleanup task
    result = cleanup_duplicate_flights.delay(dry_run=dry_run)
